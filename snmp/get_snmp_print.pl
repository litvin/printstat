...
foreach (@hosts) {
	...
	my $model = "";
		...
		my $maxt =  var_snmp($hostname, $maxToner);
		my $curt =  var_snmp($hostname, $curToner);
		...
		insert into snmp_log (l_user, l_host, l_model, l_serial, l_black, l_color, l_maxt, l_curt )
		VALUES ('root', '$hostname' , '$model', '$s', '$b', '$c', '$maxt', '$curt');
		");
		$ins->execute();		
